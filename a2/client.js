...
export default class Client {
    async main() {
        ...
        let rooms = new Map();
        let roomPrompt;
        let getRoomChoices = () => ([...rooms.values()]
            .map(room => ({
        ...
        dht.on('peer', async (peer, infoHash, from) => {
            ...
            try {
                ...
                try {
                    let serverRooms = await response(ws, 'get_rooms');
        ...
        let serverPlayers = (await response(room.server.ws, 'join_room', room.id));
        ...
        let { host, port, blowfish, playerID } = await message(room.server.ws, 'launch_client');
